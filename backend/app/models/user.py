...
class SysUser(Base):
    ...
    display_tags = Column(String(255), nullable=False, default="", server_default="", comment="展示标签，多个逗号分隔，用于分配时按标签代称展示")
...
class SysRole(Base):
    ...
    tag_only_view = Column(SmallInteger, nullable=False, default=0, server_default="0", comment="仅看分配标签：1时该角色查看任务/Bug只见标签、看不到真实指派人")
    is_enabled = Column(SmallInteger, default=1, comment="是否启用：0禁用/1启用")
    created_at = Column(DateTime, default=datetime.now, comment="创建时间")
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now, comment="更新时间")
